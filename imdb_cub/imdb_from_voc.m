...
if strcmp(image_set, 'train')
    cache_file = prcnn.dir.IMDB_TRAIN_FILE;
elseif strcmp(image_set, 'test')
    cache_file = prcnn.dir.IMDB_TEST_FILE;
...
try
  ...
  classdir = [BIRD_DIR 'classes.txt'];
  [class_id classes] = textread(classdir, '%d %s');
  imdb.parts = prcnn.config.parts;
  imdb.num_parts = prcnn.config.N_parts;
