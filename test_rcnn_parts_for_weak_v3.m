...
function boxes = test_rcnn_parts_for_weak_v3(config)

% assume they are all the same
cache_file = ['imdb_weak.mat'];
load(cache_file);

figure_dir = 'results/figure_weak';

load('caches/geo_prior.mat');
...
% P = randperm(nim);
for i = 1 : nim
    pdFile = sprintf('rcnn/feat_cache/v1_finetune_cub_train_%s_iter_10k/%s/%s_pdbox.mat',...
        parts{1}, imdb.name, imdb.image_ids{i});
    if exist(pdFile, 'file') && 0
        load(pdFile);
    ...
    fprintf('test %d/%d\n', i, nim);
    %try
    d = load(sprintf('rcnn/feat_cache/v1_finetune_cub_train_%s_iter_10k/%s/%s_score.mat',...
        parts{1}, imdb.name, imdb.image_ids{i}));
    zs = d.score;
    boxes_ = d.boxes;
    %gt_flag = d.class==0;
    ...
    [zs_s, inds_s] = sort(zs_, 'descend');
    inds_s = inds_s(1:min(100, length(inds_s)));
    
    scores = ones(length(inds_s), config.N_parts+1, config.N_methods);
    scores_idx = zeros(length(inds_s), length(parts) - 1, config.N_methods);
    max_ps = zeros(length(inds_s), length(parts) - 1);
    %neighbor_prior = fit_neighbors(knn_idx, feat_opts.X);
    
    for kk = 1 : length(inds_s)
        k = inds_s(kk);
        % fix one root filter
        ...
        s_(1,2,2) = zs_n(k);
        for p = 1 : length(parts) - 1
            zz = zp_n(I_p, p) .* (pdf(feat_opts.prior{p}, n_boxes) .^ 0.05);
    ...
    for m = 1 : config.N_methods
        [max_score, argmax] = max(scores(:, 1, m));
        ...
        if box_counts(m) > max_per_set
            top_scores{m}(max_per_set+1:end) = [];
            thresh(m) = top_scores{m}(end);
        end
        
        boxes{1,m}(i, :) = [boxes_(inds_s(argmax), :) scores(argmax,2,m)];
        for p = 1 : length(parts) - 1
            boxes{p+1,m}(i, :) = [boxes_(scores_idx(argmax, p, m), :) scores(argmax,p+2,m)];
        end
    end
...
detect_boxes = boxes;
save('caches/rcnn_detect_boxes_weak.mat', 'detect_boxes');
end
