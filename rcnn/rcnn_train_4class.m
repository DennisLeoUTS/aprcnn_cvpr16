function [rcnn_model, rcnn_k_fold_model] = ...
    rcnn_train_4class(imdb, varargin)
...
ip.addParamValue('part_name',    'bbox',     @isstr);
...
if strcmp(opts.part_name, 'bbox')
    opts.partid = 1;
elseif strcmp(opts.part_name, 'head')
    opts.partid = 2;
elseif strcmp(opts.part_name, 'body')
    opts.partid = 3;
end

% exist then load, no re-computation
rcnn_model_file = sprintf(conf.dir.RCNN_DETECT_MODEL_FILE, imdb.name, opts.part_name);
if exist(rcnn_model_file, 'file')
    load(rcnn_model_file);
    if opts.k_folds > 0
        load([conf.cache_dir 'rcnn_k_fold_model_' opts.part_name '.mat']);
    else
        rcnn_k_fold_model = [];
    end
    return;
...
[~,cache_name,~] = fileparts(opts.cache_name);
rcnn_model = rcnn_create_model(opts.net_def_file, opts.net_file, cache_name);
...
save_file = sprintf(conf.dir.GT_POS_FEATURE_FILE, ...
...
X_pos = rcnn_pool5_to_fcX(X_pos, opts.layer, rcnn_model);
X_pos = rcnn_scale_features(X_pos, opts.feat_norm_mean);
caches = init_cache(X_pos, keys_pos);
% ------------------------------------------------------------------------
    ...
    for i = 1:length(imdb.image_ids)
        ...
        %for j = opts.partid%imdb.class_ids
            ...
            if (first_time || hit_retrain_limit || is_last_time) && ...
                    ~isempty(caches.X_neg)
                fprintf('>>> Updating %s detector <<<\n', opts.part_name);
                fprintf('Cache holds %d pos examples %d neg examples\n', ...
                    size(caches.X_pos,1), size(caches.X_neg,1));
                [new_w, new_b] = update_model(caches, opts);
                rcnn_model.detectors.W = new_w;
                rcnn_model.detectors.B = new_b;
                caches.num_added = 0;
                
                z_pos = caches.X_pos * new_w + new_b;
                z_neg = caches.X_neg * new_w + new_b;
                
                caches.pos_loss(end+1) = opts.svm_C * opts.pos_loss_weight * ...
                    sum(max(0, 1 - z_pos));
                caches.neg_loss(end+1) = opts.svm_C * sum(max(0, 1 + z_neg));
                caches.reg_loss(end+1) = 0.5 * new_w' * new_w + ...
                    0.5 * (new_b / opts.bias_mult)^2;
                caches.tot_loss(end+1) = caches.pos_loss(end) + ...
                    caches.neg_loss(end) + ...
                    caches.reg_loss(end);
                
                for t = 1:length(caches.tot_loss)
                ...
                rcnn_model.SVs.keys_neg = caches.keys_neg(SVs_neg, :);
                rcnn_model.SVs.scores_neg = z_neg(SVs_neg);
        ...
        if opts.checkpoint > 0 && mod(i, opts.checkpoint) == 0
            save(rcnn_model_file, 'rcnn_model');
        end
    end
end
...
if opts.k_folds > 0
    ...
    save([conf.cache_dir 'rcnn_k_fold_model_' opts.part_name '.mat'], 'rcnn_k_fold_model');
else
    rcnn_k_fold_model = [];
end
% ------------------------------------------------------------------------
...
if first_time
    I = find(d.overlap(:, opts.partid) < neg_ovr_thresh);
    X_neg = d.feat(I,:);
    keys = [ind*ones(length(I),1) I];
else
    zs = bsxfun(@plus, d.feat*rcnn_model.detectors.W, rcnn_model.detectors.B);

    z = zs;
    I = find((z > caches.hard_thresh) & ...
        (d.overlap(:, opts.partid) < neg_ovr_thresh));
...
for i = 1:length(imdb.image_ids)
    ...
    sel = find(d.class == opts.partid);  %1: bbox
    
    if ~isempty(sel)
        X_pos = cat(1,X_pos,d.feat(sel,:));
        keys = cat(1,keys,[i, sel]);
    end
end
