...
testl = config.testlabel;

try
    aboxes = cell(num_classes, 1);
    for i = 1:num_classes
        load([conf.cache_dir rcnn_model.classes{i} '_boxes_' imdb.name suffix '.mat1']);
        aboxes{i} = boxes;
    end
catch
    aboxes = cell(num_classes, 1);
    ...
    for f = 1:length(folds)
        for i = folds{f}
            ...
            partid = 1;
            zs = zs(d.gt==0);
            overlap = d.overlap(d.gt==0,:);
            [x, maxc] = max(zs);
            ov = overlap(maxc,partid+1);
            if ~isempty(find(d.class==partid))
                pcps = [pcps; ov>.5];
            end
            acc = mean(pcps);
            fprintf('Testing example %d, overlap: %.4f, score: %.4f, acc: %.4f\n', i, ov, x, acc);
...
%             end
    ...
    for i = 1:num_classes
        ...
        save_file = [conf.cache_dir rcnn_model.classes{i} '_boxes_' imdb.name suffix '.mat'];
        boxes = aboxes{i};
        inds = box_inds{i};
