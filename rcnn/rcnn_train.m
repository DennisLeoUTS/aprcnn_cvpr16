...
if exist([conf.cache_dir 'rcnn_model_' opts.part_name '.mat'], 'file')
    load([conf.cache_dir 'rcnn_model_' opts.part_name '.mat']);
    if opts.k_folds > 0
...
if strcmp(imdb.name, 'parts_train')
    opts.labels = config.trainlabel;
else
    opts.labels = config.testlabel;
end
...
for hard_epoch = 1:max_hard_epochs
    for i = 1:length(imdb.image_ids)
        ...
        if opts.checkpoint > 0 && mod(i, opts.checkpoint) == 0
            save([conf.cache_dir 'rcnn_model_' opts.part_name '.mat'], 'rcnn_model');
        end
    end
end
...
if first_time
    for cls_id = class_ids
        if cls_id==opts.labels(ind)  % gt class
            I = find(d.overlap(:, opts.partid) < neg_ovr_thresh);
        else
            I = 1:size(d.overlap,1);
            I = I';
        end
...
for i = 1:length(imdb.image_ids)
    ...
    if ~isempty(sel)
        X_pos{l} = cat(1,X_pos{l},d.feat(sel,:));
        keys{l} = cat(1,keys{l},[i, sel]);
    end
    %   for j = imdb.class_ids
    %     if isempty(X_pos{j})
    %       X_pos{j} = single([]);
