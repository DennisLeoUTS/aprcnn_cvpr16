function rcnn_compute_score(imdb, detector_model, varargin)
...
opts.output_dir = fullfile(conf.dir.RCNN_FEATURE_PATH, imdb.name);
...
load(sprintf(conf.dir.RCNN_FEATURE_STAT_FILE, ...
                    'parts_train', 'parts_train', opts.layer, cache_name));
...
for i = opts.start:opts.end
    ...
    save_file = sprintf(conf.dir.RCNN_DETECT_SCORE_FILE, imdb.name, image_ids{i});
    ...
    pool5_file = sprintf(conf.dir.RCNN_POOL5_FILE, imdb.name, image_ids{i});
