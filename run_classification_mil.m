...
function results = run_classification_mil(MODE, ITER)
...
try
    load(prcnn.dir.TRAIN_MODEL_MIL_FILE);
catch
    try
    ...
    if MODE==2
        ...
        try 
            ...
            if exist(prcnn.dir.WEAK_DETECT_BOX_MIL_FILE, 'file')
                load(prcnn.dir.WEAK_DETECT_BOX_MIL_FILE);
            else
                detect_boxes = test_rcnn_parts_for_weak_mil(prcnn);
            end
            ...
            imweakpath_aug = reshape(repmat(imweakpath,10,1),1,Nim*10);
            
            for i = 1 : prcnn.config.N_parts
                db = detect_boxes{i, prcnn.para.selected_geo_method};
                db = reshape(permute(db,[2,1,3]),size(db,1)*size(db,2),size(db,3));
                weak_fea{i} = zeros(Nim*10,4096);
                ind = find(db(:,1)~=-1);
                Nseg = ceil(length(ind)/10);
                for tt=1:10
                    ind1 = ind((tt-1)*Nseg+1:min(tt*Nseg,length(ind)));
                    ww = extract_deep_feature(prcnn.dir.CNN_MODELS{i}, ...
                        prcnn.dir.MODEL_DEF_FILE, imweakpath_aug(ind1), db(ind1,:));
                    weak_fea{i}(ind1,:) = ww;
                end
            end
            save(prcnn.dir.WEAK_FEA_MIL_FILE, 'weak_fea', '-v7.3');
        end
        WEAK_fea = [];
        for i = 1 : prcnn.config.N_parts
    ...
    save(prcnn.dir.TRAIN_MODEL_MIL_FILE, 'model');
