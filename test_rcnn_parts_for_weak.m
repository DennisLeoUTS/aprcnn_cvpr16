...
function boxes = test_rcnn_parts_for_weak(prcnn)

if exist(prcnn.dir.WEAK_DETECT_BOX_FILE, 'file') && prcnn.para.canSkip
...
for i = 1 : nim
    pdFile = sprintf(prcnn.dir.WEAK_DETECT_BOX_IMAGE_FILE, parts{1}, imdb.name, imdb.image_ids{i});
    ...
    for m = 1 : config.N_methods
        ...
        boxes{1,m}(i, :) = [boxes_(argmax, :) scores(argmax,2,m)];
        for p = 1 : length(parts) - 1
...
detect_boxes = boxes;
save(prcnn.dir.WEAK_DETECT_BOX_FILE, 'detect_boxes');
end
