...
% init rcnn model
% disp('before clear caffe...');
% clear caffe;
% disp('before init caffe...');
% matcaffe_init(0, model_def, model_file);
% disp('Caffe init successfully!');
rcnn_model = rcnn_create_model(model_def, model_file);
...
d = load(prcnn.dir.CAFFE_MEAN_FILE);
...
for batch = 1 : num_batches
  ...
  for j = batch_start:batch_end
    ...
    %rectangle('Position',[bbox(1),bbox(2),bbox(3)-bbox(1),bbox(4)-bbox(2)],'EdgeColor','r');
    if size(im,3) ~= 3
    ...
    crop = rcnn_im_crop(im, bbox, crop_mode, crop_size, ...
        ...
        crop = imresize(im, [crop_size crop_size]);
    end
